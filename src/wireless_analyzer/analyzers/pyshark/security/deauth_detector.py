"""
PyShark-specific Deauthentication Flood and Attack Detection Analyzer

Detects deauthentication flood attacks, targeted attacks, and unusual deauth patterns
using PyShark's native packet parsing capabilities.
"""
...
class PySharkDeauthFloodDetector(BasePySharkAnalyzer):
    """
    PyShark-based deauthentication flood and attack detector.
    ...
    def __init__(self):
        super().__init__()
        self.name = "PyShark Deauthentication Flood Detector"
        self.description = "Detects deauth floods and attacks using PyShark parsing"
        self.version = "1.0.0"
    ...
    def analyze_packet(self, packet, metadata: Dict[str, Any] = None) -> List[Finding]:
        ...
        try:
            if not self._is_deauth_packet(packet):
                return findings
    ...
    def _is_deauth_packet(self, packet) -> bool:
        """Check if packet is a deauthentication frame."""
        try:
            if not hasattr(packet, 'wlan'):
                return False
            
            # Check frame type and subtype for deauth
            if hasattr(packet.wlan, 'fc_type') and hasattr(packet.wlan, 'fc_subtype'):
                ...
                # Management frame (type 0) with deauth subtype (12)
                return frame_type == 0 and subtype == 12
    ...
    def _extract_deauth_info(self, packet) -> Dict[str, Any]:
        """Extract deauth packet information using PyShark."""
        try:
            if not hasattr(packet, 'wlan'):
                return None
            ...
            # Extract reason code from management layer
            reason = 0
            if hasattr(packet, 'wlan_mgt') and hasattr(packet.wlan_mgt, 'reason_code'):
                try:
                    reason = int(packet.wlan_mgt.reason_code)
