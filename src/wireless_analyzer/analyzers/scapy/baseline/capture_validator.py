"""
Scapy-based capture quality analysis and validation for wireless PCAP data.

This analyzer validates the quality and completeness of wireless packet captures using
native Scapy packet parsing:
...
from scapy.layers.dot11 import (
    Dot11, Dot11Beacon, Dot11ProbeResp, Dot11ProbeReq, Dot11Auth, Dot11Deauth,
    Dot11AssoReq, Dot11AssoResp, Dot11ReassoReq, Dot11ReassoResp, Dot11Disas,
    Dot11QoS, Dot11CCMP, Dot11WEP, RadioTap
)
...
class ScapyCaptureQualityAnalyzer(BaseScapyAnalyzer):
    """
    Scapy-based comprehensive capture quality and validation analyzer.
    
    This analyzer assesses the quality and completeness of wireless packet captures
    using native Scapy packet parsing to ensure reliable analysis results. It validates:
    ...
    def __init__(self):
        super().__init__(
            name="Scapy Capture Quality Validator",
            category=AnalysisCategory.CAPTURE_QUALITY,
            version="1.0"
        )
        self.description = "Validates monitor mode, timing, FCS inclusion, and capture integrity using Scapy"
    ...
    def analyze(self, packets: List[Packet], context: AnalysisContext) -> List[Finding]:
        """
        Analyze packets for capture quality and integrity using Scapy.
        ...
        self.logger.info(f"Analyzing capture quality of {len(packets)} packets using Scapy")
    ...
    def _collect_quality_metrics(self, packets: List[Packet]) -> None:
        """Collect quality metrics from all packets using Scapy."""
        packet_signatures = []
    ...
    def _analyze_radiotap_header(self, packet: Packet) -> None:
        """Analyze RadioTap header presence and quality using Scapy."""
    ...
    def _analyze_dot11_frame(self, packet: Packet, packet_index: int) -> None:
        """Analyze 802.11 frame for quality indicators using Scapy."""
        dot11 = packet[Dot11]
    ...
    def _check_fcs_presence(self, packet: Packet) -> None:
        """Check for FCS presence and validity using Scapy."""
        # Look for FCS-related attributes in Scapy packet
        has_fcs = False
        fcs_valid = False
        
        # Common FCS indicators in Scapy packet structures
    ...
    def _check_monitor_mode_indicators(self, packet: Packet) -> None:
        """Check for indicators that suggest monitor mode operation using Scapy."""
    ...
    def _collect_hardware_indicators(self, packet: Packet) -> None:
        """Collect hardware and driver capability indicators using Scapy."""
    ...
    def _analyze_monitor_mode(self) -> List[Finding]:
        ...
        if management_ratio < self.min_management_frames_ratio:
            findings.append(self.create_finding(
                ...
                title="Monitor Mode Not Detected (Scapy)",
            ...
            findings.append(self.create_finding(
                ...
                title="Low Management Frame Ratio (Scapy)",
        ...
        if len(unique_indicators) >= 3:
            findings.append(self.create_finding(
                ...
                title="Monitor Mode Confirmed (Scapy)",
    ...
    def _analyze_timing_quality(self) -> List[Finding]:
        ...
        if max_interval > 10.0:  # Gaps > 10 seconds
            findings.append(self.create_finding(
                severity=Severity.WARNING,
                title="Large Timing Gaps Detected (Scapy)",
        ...
        if len(microsecond_timestamps) / len(self.timestamps) > 0.8:
            findings.append(self.create_finding(
                severity=Severity.INFO,
                title="High-Precision Timestamps Detected (Scapy)",
        ...
        if interval_std > avg_interval * 2 and avg_interval > 0:
            findings.append(self.create_finding(
                severity=Severity.WARNING,
                title="Inconsistent Packet Timing (Scapy)",
    ...
    def _analyze_fcs_inclusion(self) -> List[Finding]:
        ...
        if fcs_ratio < self.min_fcs_inclusion_ratio:
            findings.append(self.create_finding(
                severity=Severity.WARNING,
                title="Low FCS Inclusion Rate (Scapy)",
            ...
            findings.append(self.create_finding(
                severity=Severity.INFO,
                title="High FCS Inclusion Rate (Scapy)",
        ...
        if self.fcs_valid_count + self.fcs_invalid_count > 0:
            ...
            if error_rate > 0.05:  # > 5% FCS errors
                findings.append(self.create_finding(
                    severity=Severity.WARNING,
                    title="High FCS Error Rate (Scapy)",
    ...
    def _analyze_radiotap_quality(self) -> List[Finding]:
        ...
        if radiotap_ratio < 0.1:  # Less than 10% have RadioTap
            findings.append(self.create_finding(
                severity=Severity.WARNING,
                title="Limited RadioTap Information (Scapy)",
            ...
            findings.append(self.create_finding(
                severity=Severity.INFO,
                title="Rich RadioTap Metadata Available (Scapy)",
    ...
    def _analyze_capture_completeness(self) -> List[Finding]:
        ...
        if self.capture_duration < self.min_capture_duration:
            findings.append(self.create_finding(
                severity=Severity.WARNING,
                title="Very Short Capture Duration (Scapy)",
        ...
        if self.capture_duration > 0:
            ...
            if packet_rate < 1:  # Less than 1 packet per second
                findings.append(self.create_finding(
                    severity=Severity.WARNING,
                    title="Low Packet Capture Rate (Scapy)",
                ...
                findings.append(self.create_finding(
                    severity=Severity.INFO,
                    title="High Packet Capture Rate (Scapy)",
    ...
    def _analyze_hardware_capabilities(self) -> List[Finding]:
        ...
        if self.hardware_indicators:
            findings.append(self.create_finding(
                severity=Severity.INFO,
                title="Advanced Hardware Capabilities Detected (Scapy)",
        ...
        if len(self.antenna_info) > 1:
            findings.append(self.create_finding(
                severity=Severity.INFO,
                title="Multiple Antenna Support Detected (Scapy)",
    ...
    def _analyze_duplicate_frames(self) -> List[Finding]:
        ...
        if duplicate_ratio > self.max_duplicate_ratio:
            findings.append(self.create_finding(
                severity=Severity.WARNING,
                title="High Duplicate Frame Rate (Scapy)",
    ...
    def _analyze_capture_setup(self) -> List[Finding]:
        ...
        if quality_score < 0.6:
            findings.append(self.create_finding(
                severity=Severity.CRITICAL,
                title="Poor Capture Quality Detected (Scapy)",
                description=f"Overall capture quality score: {quality_score:.2f}/1.00",
                details=dict(self._get_quality_summary(), parser="scapy"),
                recommendations=[
            ...
            findings.append(self.create_finding(
                severity=Severity.WARNING,
                title="Suboptimal Capture Quality (Scapy)",
            ...
            findings.append(self.create_finding(
                severity=Severity.INFO,
                title="Good Capture Quality (Scapy)",
    ...
    def _create_packet_signature(self, packet: Packet) -> str:
        """Create a simple signature for duplicate detection using Scapy."""
        try:
            if packet.haslayer(Dot11):
                dot11 = packet[Dot11]
    ...
    def _extract_timestamp(self, packet: Packet) -> float:
        """Extract timestamp from packet using Scapy."""
