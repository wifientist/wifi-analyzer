"""
Scapy-based Beacon Analysis for wireless PCAP data.

This analyzer provides comprehensive beacon frame analysis using native Scapy
...
class BeaconInfo(NamedTuple):
    """Information extracted from a beacon frame using Scapy."""
    timestamp: float
    bssid: str
    ssid: str
    beacon_interval: int
...
class ScapyBeaconAnalyzer(BaseScapyAnalyzer):
    """
    Scapy-based comprehensive beacon frame analyzer.
    ...
    def __init__(self):
        super().__init__("Scapy Beacon Frame Analyzer", AnalysisCategory.BEACONS, "1.0")
        
        self.description = (
            "Analyzes 802.11 beacon frames for timing consistency, "
            "information element validation, and capability analysis using Scapy"
        )
    ...
    def analyze(self, packets: List[Packet], context: AnalysisContext) -> List[Finding]:
        ...
        self.logger.info(f"Analyzing {len(packets)} beacon frames with Scapy")
        
        findings = []
        
        # Extract beacon information using Scapy
        beacon_data = self._extract_beacon_info_scapy(packets)
    ...
    def _extract_beacon_info_scapy(self, packets: List[Packet]) -> List[BeaconInfo]:
        """Extract beacon information from Scapy packets."""
        beacon_info = []
        processed_count = 0
        error_count = 0
        ...
        for packet in packets:
            try:
                ...
                # Extract SSID and all IEs using Scapy
                ssid = ""
                ies = {}
                if packet.haslayer(Dot11Elt):
                    current_ie = packet[Dot11Elt]
                    ie_count = 0
                    while current_ie and ie_count < 50:  # Prevent infinite loops
                        try:
                            ...
                            self.logger.debug(f"Error parsing IE in beacon: {e}")
                            break
                ...
                error_count += 1
                self.logger.debug(f"Error extracting beacon info from packet: {e}")
                continue
    ...
    def _analyze_beacon_timing(self, bssid: str, beacons: List[BeaconInfo]) -> List[Finding]:
        """Analyze beacon timing consistency using Scapy data."""
        findings = []
    ...
    def _analyze_information_elements(self, bssid: str, beacons: List[BeaconInfo]) -> List[Finding]:
        """Analyze Information Elements in beacon frames using Scapy data."""
        findings = []
    ...
    def _analyze_capabilities(self, bssid: str, beacons: List[BeaconInfo]) -> List[Finding]:
        """Analyze capability fields in beacon frames using Scapy data."""
        findings = []
        ...
        # Convert capabilities to int to handle Scapy FlagValue objects
    ...
    def _analyze_network_config(self, bssid: str, beacons: List[BeaconInfo]) -> List[Finding]:
        """Analyze network configuration from beacon data using Scapy."""
        findings = []
    ...
    def _analyze_network_conflicts(self, ap_beacons: Dict[str, List[BeaconInfo]]) -> List[Finding]:
        """Analyze conflicts between different networks using Scapy data."""
        findings = []
    ...
    # IE Validation Methods (Scapy-specific implementations)
    def _validate_ssid_ie(self, bssid: str, ssid: str, ie_id: int, ie_data: bytes) -> List[Finding]:
        """Validate SSID Information Element using Scapy data."""
        findings = []
    ...
    def _validate_rates_ie(self, bssid: str, ssid: str, ie_id: int, ie_data: bytes) -> List[Finding]:
        """Validate Supported Rates Information Element using Scapy data."""
        findings = []
    ...
    def _validate_channel_ie(self, bssid: str, ssid: str, ie_id: int, ie_data: bytes) -> List[Finding]:
        """Validate DS Parameter Set (Channel) Information Element using Scapy data."""
        findings = []
    ...
    def _validate_tim_ie(self, bssid: str, ssid: str, ie_id: int, ie_data: bytes) -> List[Finding]:
        """Validate Traffic Indication Map Information Element using Scapy data."""
        findings = []
    ...
    def _validate_country_ie(self, bssid: str, ssid: str, ie_id: int, ie_data: bytes) -> List[Finding]:
        """Validate Country Information Element using Scapy data."""
        findings = []
