"""
Scapy-based Authentication & Association Flow Analysis for wireless PCAP data.

This analyzer provides comprehensive authentication and association flow analysis using native Scapy
...
from scapy.layers.dot11 import (
    ...
    Dot11Deauth, Dot11Disas, Dot11Elt, RadioTap
)
...
class ScapyAuthAssocFlowAnalyzer(BaseScapyAnalyzer):
    """
    Scapy-based Authentication & Association Flow Analyzer.
    ...
    def __init__(self):
        super().__init__(
            name="Scapy Auth/Assoc Flow Analyzer",
            category=AnalysisCategory.AUTH_ASSOC,
            version="1.0"
        )
        
        self.description = (
            "Analyzes authentication and association flows using Scapy, including timing, "
    ...
    def analyze(self, packets: List[Packet], context: AnalysisContext) -> List[Finding]:
        """
        Analyze authentication and association flows using Scapy.
        ...
        self.logger.info(f"Analyzing auth/assoc flows from {len(packets)} packets using Scapy")
    ...
    def _extract_connection_frame(self, packet: Packet) -> Optional[ConnectionFrame]:
        """Extract connection frame information using native Scapy parsing."""
        try:
    ...
    def _extract_supported_rates(self, packet: Packet) -> List[str]:
        """Extract supported rates from packet IEs using Scapy."""
        rates = []
    ...
    def _analyze_connection_success_rates(self) -> List[Finding]:
        ...
        findings.append(self.create_finding(
            severity=severity,
            title="Connection Success Rate Analysis (Scapy)",
        ...
        if low_success_stations:
            findings.append(self.create_finding(
                ...
                title="Stations with Low Connection Success Rates (Scapy)",
    ...
    def _analyze_connection_timing(self) -> List[Finding]:
        ...
        if timing_issues:
            findings.append(self.create_finding(
                severity=Severity.WARNING,
                title="Connection Timing Performance Issues (Scapy)",
        ...
        if total_times:
            findings.append(self.create_finding(
                ...
                title="Connection Timing Analysis (Scapy)",
    ...
    def _analyze_failure_patterns(self) -> List[Finding]:
        ...
        findings.append(self.create_finding(
            severity=Severity.WARNING,
            title="Connection Failure Analysis (Scapy)",
        ...
        if critical_failures:
            findings.append(self.create_finding(
                ...
                title="Critical Connection Failure Patterns (Scapy)",
    ...
    def _analyze_retry_behavior(self) -> List[Finding]:
        ...
        if retry_counts:
            ...
            findings.append(self.create_finding(
                ...
                title="Retry Behavior Analysis (Scapy)",
        ...
        if high_retry_attempts:
            findings.append(self.create_finding(
                severity=Severity.WARNING,
                title="High Retry Connection Attempts (Scapy)",
    ...
    def _analyze_capability_negotiation(self) -> List[Finding]:
        ...
        if rate_negotiations:
            findings.append(self.create_finding(
                severity=Severity.WARNING,
                title="Rate Negotiation Issues (Scapy)",
    ...
    def _analyze_roaming_behavior(self) -> List[Finding]:
        ...
        if roaming_stations:
            ...
            findings.append(self.create_finding(
                severity=Severity.INFO,
                title="Client Roaming Behavior Analysis (Scapy)",
